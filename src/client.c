...
void create_child_process(const int child_id) {
	char child_sub_endpoint[MAX_LEN]; // parent --> child
	bool is_left = child_id < CLIENT_ID;
	if (is_left) {
		strcpy(child_sub_endpoint, LEFT_PUB_ENDPOINT);
	} else {
		strcpy(child_sub_endpoint, RIGHT_PUB_ENDPOINT);
	}
	char child_pub_endpoint[MAX_LEN]; // parent <-- child
	create_endpoint(child_pub_endpoint, child_id, client_parent_pub);
	...
	if (fork_pid < 0) {
		...
		execl(CLIENT, CLIENT, child_id_str, parent_id_str, child_sub_endpoint, child_pub_endpoint, NULL);
	} else {
		if (is_left) {
			strcpy(LEFT_SUB_ENDPOINT, child_pub_endpoint); // Cur Client reads from left child
			HAS_LEFT = true;
		} else {
			strcpy(RIGHT_SUB_ENDPOINT, child_pub_endpoint);
			HAS_RIGHT = true;
		}
		
		bind_zmq_socket(SUBSCRIBER, child_pub_endpoint);
	}
}
...
// [id, parent_id, parent_sub_end, parent_pub_end]
