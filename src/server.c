...
void init_server() {
    ...
    create_endpoint(PUB_ENDPOINT, 0, server_pub); // PUB_ENDP - путь на котором создастся сокет
    bind_zmq_socket(PUBLISHER, PUB_ENDPOINT); // Сокет должен писать в путь PUB_ENDPOINT
...
void create_child_process(const int client_id) {
    ...
    if (!FIRST_CLIENT_CONNECTED) {
        ...
        if (fork_pid < 0) {
            ...
            execl(CLIENT, CLIENT, client_id_str, "-1", parent_sub_endpoint, parent_pub_endpoint, NULL);
        } else {
